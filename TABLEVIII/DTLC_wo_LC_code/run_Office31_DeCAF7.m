...
source_domains = {'Amazon_fc7', 'Amazon_fc7', 'Dslr_fc7', 'Dslr_fc7', 'Webcam_fc7', 'Webcam_fc7'};
target_domains = {'Dslr_fc7', 'Webcam_fc7', 'Amazon_fc7', 'Webcam_fc7', 'Amazon_fc7', 'Dslr_fc7'};
result = [];
...
for iData = 1:length(target_domains)
	...
	Xt = Xt';
    Yt = labels;
    fprintf('DTLC without label consistency:  data=%s alpha=%f beta=%f eta=%f\n', options.data, options.alpha, options.beta, options.eta);
    ...
    for t = 1:T
        ...
        fprintf('DTLC without label consistency = %0.4f\n', acc);
        Acc = [Acc; acc(1)];
    end
    Cls2=Cls;
    savefile = ['save_2\Y_',source,'_',target,'.mat'];
    save(savefile,'Cls2');
